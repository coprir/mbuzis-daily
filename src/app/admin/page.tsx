...
export default function Admin() {
  const {
    users, rooms, requests, modLogs, activity, emailEnabled,
              ...
              <div className="space-y-2">
                {admins.map((a) => (
                  <div key={a.id} className="flex items-center gap-3 rounded-2xl border border-sand-50/[0.07] bg-sand-50/[0.03] p-2.5">
                    ...
                    {a.owner ? (
                      <span className="chip border border-honey-500/30 bg-honey-500/10 text-honey-400"><Crown className="h-3 w-3" /> super admin</span>
                    ) : (
                      <>
